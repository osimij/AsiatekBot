...
# ---------- ENVIRONMENT ----------
REQ = ["TELEGRAM_BOT_TOKEN","SUPABASE_URL","SUPABASE_KEY",
...
# ---------- CLIENTS ----------
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)  # sync client
...
async def _insert_async(table: str, data: Dict[str, Any]) -> None:
    """Non-blocking Supabase insert (runs in thread pool)."""
    await asyncio.to_thread(
        supabase.table(table).insert(data, returning=None).execute
    )

async def log_interaction(update: Optional[Update], context: ContextTypes.DEFAULT_TYPE,
                          interaction_type: str, detail: Optional[str]=None,
    ...
    elif user_id_override: user_id=user_id_override
    if not user_id: return
    payload = {k:v for k,v in {
        "user_id":user_id,"username":username,"first_name":first_name,
        "interaction_type":interaction_type,"interaction_detail":detail
...
async def send_admin_notification(user: dict, order: dict):
    html = f"""
    <h2>Получен новый запрос на автозапчасти</h2><hr>
    <p><b>ID:</b> {user['id']}</p>
    <p><b>Username:</b> @{user.get('username','нет')}</p>
    ...
    await asyncio.to_thread(
        resend.Emails.send,
        {"from":"Parts Bot <bot@asiatek.pro>","to":[ADMIN_EMAIL],
         "subject":"Новый запрос на автозапчасти","html":html}
    )

async def save_order_to_supabase(**data) -> bool:
    try:
        await _insert_async("orders",{k:v for k,v in data.items() if v})
        return True
    except Exception as e:
        logger.error(f"Supabase insert failed: {e}"); return False

# ---------- HANDLERS (same logic, slimmed comments) ----------
...
async def ask_vin_known_handler(update: Update, context: ContextTypes.DEFAULT_TYPE)->int:
    ...
    if q.data=="vin_yes":
        await q.edit_message_text("Введите ваш 17-значный VIN:"); return GET_VIN
...
async def get_vin(update: Update, context: ContextTypes.DEFAULT_TYPE)->int:
    ...
    if not re.fullmatch(r"[A-HJ-NPR-Z0-9]{17}",vin):
        await update.message.reply_text("Неверный VIN. Попробуйте ещё.")
        return GET_VIN
    context.user_data['vin']=vin
    await update.message.reply_text("Спасибо! Теперь контакт:")
    return GET_CONTACT

async def get_contact(update: Update, context: ContextTypes.DEFAULT_TYPE)->int:
    contact=update.message.text.strip()
    if len(contact)<5:
        await update.message.reply_text("Некорректно. Введите телефон/e-mail.")
        return GET_CONTACT
    context.user_data['contact']=contact
    await update.message.reply_text("Опишите нужные запчасти:")
    return GET_PARTS
...
async def get_parts(update: Update, context: ContextTypes.DEFAULT_TYPE)->int:
    ...
    ok=await save_order_to_supabase(
        telegram_user_id=data['id'], telegram_username=data['username'],
        vin=data.get('vin'), contact=data['contact'], parts=parts)
    if ok:
        await send_admin_notification(
            {"id":data['id'],"username":data['username']},
            {"vin":data.get('vin'),"contact":data['contact'],"parts":parts})
        txt="✅ Запрос сохранён. Мы свяжемся с вами!"
    else: txt="❌ Ошибка сохранения. Попробуйте позже."
...
async def fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await log_interaction(update,context,'fallback',(update.message.text or '')[:100])
...
def main()->None:
    ...
    conv = ConversationHandler(
        [CommandHandler("start",start), CallbackQueryHandler(start,pattern="^new_request$")],
    ...
    app.web_app.router.add_get("/healthz", keep_alive)   # same handler
