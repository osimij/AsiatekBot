...
async def log_interaction(update:Optional[Update], context:ContextTypes.DEFAULT_TYPE,
    ...
    if update and update.effective_user:
        u=update.effective_user; user_id,username,first_name=u.id,u.username,u.first_name
...
async def send_admin_notification(user:dict, order:dict):
    html=f"""<h2>Получен новый запрос</h2><hr>
    <p><b>ID:</b> {user['id']}</p>
...
async def save_order_to_supabase(**data)->bool:
    try:
        await _insert_async("orders",{k:v for k,v in data.items() if v}); return True
...
async def start(update:Update, context:ContextTypes.DEFAULT_TYPE)->int:
    ...
    context.user_data.clear(); context.user_data.update({"id":u.id,"username":u.username,"vin":None})
...
async def get_parts(update:Update, context:ContextTypes.DEFAULT_TYPE)->int:
    parts=update.message.text.strip(); d=context.user_data
    ok=await save_order_to_supabase(
        telegram_user_id=d['id'], telegram_username=d['username'],
        vin=d.get('vin'), contact=d['contact'], parts=parts)
    if ok:
        await send_admin_notification({"id":d['id'],"username":d['username']},
                                      {"vin":d.get('vin'),"contact":d['contact'],"parts":parts})
        msg="✅ Запрос сохранён. Мы свяжемся с вами!"
    else: msg="❌ Ошибка сохранения. Попробуйте позже."
